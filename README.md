...
This is a tool for mirroring a YouTube channel to a BlueSky account. Once deployed, the tool will update the BlueSky profile picture, banner, display name, and description to match the YouTube channel. It will also make posts when any videos, shorts, or community posts are made by the YouTube channel.
...
Define the environment variables in an env file and make sure the path is correct in the env_file node of the docker compose file, then run compose.
...
#### Direct
 ...
 To run the project directly, create a virtual environment and install the dependencies using poetry:
 ```powershell
pip install poetry
...
```powershell
poetry run python bluesky-youtube-mirror.py
```
